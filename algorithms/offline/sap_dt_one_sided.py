"""Reference-anchored, one-sided state-aligned preference DT experiment.

The first 50k updates are identical to CORL DT.  Afterwards, the preference
objective can only reduce error on a preferred action: the non-preferred action
is a detached comparison target and is never pushed away.  A frozen DT snapshot
anchors predictions at the CORL evaluation return so preference fine-tuning
cannot cheaply destroy the original high-return policy.
"""
